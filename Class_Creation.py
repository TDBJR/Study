class Athlete: #<----When creating a class you only need : instead of ():
    ...
    def top3(self):
        return(sorted(set([Convert(t) for t in self.times]))[0:3])
    ...
    def add_times(self,list_of_times): # even though this will be a list you do not need to set it to =[]
        self.times.extend(list_of_times) # .extend() is how you add multiple items to a list at once
...
def OpenXL(filename):
    try:
        with open(filename) as f:
            ...
            return(Athlete(splitter.pop(0), splitter.pop(0), splitter))#<- This line is calling the class creation Athlete()
...
vera = Athlete('Vera Vi') # testing methods
vera.add_time('1.31')
print(vera.top3())
